...
/**
 * Multi-page tour driven by React Router.
 *
 * The library has no built-in router coupling; the wiring is two small pieces:
 *
 * 1. Each step carries the route it lives on in `metadata.route`.
 * 2. A per-flow `onStepChange` callback calls the router's `navigate()` whenever
 *    the active step's route differs from the current location.
 *
 * The provider runs with `targetNotFound="wait"`, so while the new page is
 * mounting (and the step's target does not exist yet) the portal simply waits.
 * As soon as the target registers, the portal resumes on that step — no manual
 * timing or retries needed.
 */

/** Shape of the per-step metadata this example uses. */
interface StepRoute {
  route: string;
}
...
const multiPageTour: Tutorial<StepRoute> = {
  ...
  steps: [
    ...
    },
    {
      ...
      title: "Your plan",
      description:
        "Now we jumped to another page. React Router navigated here and the tour resumed automatically once this card mounted.",
 ...
 * step. `navigate`/`location` are read through refs so the registered callback
  ...
  return (
    <div style={card}>
      ...
      <TutorialTarget id="users-create">
        <button style={demoPrimaryButton}>Invite user</button>
      </TutorialTarget>
    </div>
  );
}
    ...
    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
      ...
      <TutorialTarget id="billing-invoice">
        <div style={card}>
          ...
          <button style={demoButton}>Download latest</button>
        </div>
      </TutorialTarget>
    </div>
  );
  ...
  return (
    <div style={card}>
      ...
      <TutorialTarget id="settings-save">
        <button style={demoPrimaryButton}>Save changes</button>
      </TutorialTarget>
    </div>
  );
}
...
function MultiPageApp({ children }: { children?: ReactNode }) {
  useRouterTour();
  return (
    <div style={shell}>
      <aside style={sidebar}>
...
const meta: Meta = {
  title: "Tutorial/Multi-page (React Router)",
};
export default meta;
...
/**
 * A single flow whose steps live on three different routes. Starting it walks
 * the user across `/users`, `/billing` and `/settings`, driving the router as
 * it goes and waiting for each page's target to mount.
 */
export const MultiPagina: Story = {
  render: () => (
