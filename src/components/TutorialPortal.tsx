...
export function TutorialPortal() {
  ...
  const centered =
    isRunning &&
    (targetId === null || (missing && config.targetNotFound === "center"));
  const waiting =
    missing &&
    (config.targetNotFound === "wait" || config.targetNotFound === "skip");
  ...
  const paddedRect =
    !centered && rect ? expandRect(rect, config.spotlightPadding) : null;
